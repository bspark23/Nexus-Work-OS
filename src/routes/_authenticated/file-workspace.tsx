...
import {
  ...
  FileText, Download, Table2, Users, User,
} from "lucide-react";
...
import { useMyDepartment, useSavedFile, useAllSavedFiles } from "@/hooks/useData";
import {
  ...
  async function handleUpload(files: FileList | null) {
    ...
    try {
      ...
      const fileType = files[0].name.toLowerCase().endsWith(".pdf")
        ? "pdf" : files[0].name.toLowerCase().endsWith(".csv")
          ? "csv" : "xlsx";
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Upload spreadsheets or PDFs, edit them, and reuse for customer jobs. Admins can view all uploaded files."
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <label className="cursor-pointer">
              ...
              <input type="file" hidden accept=".xlsx,.xls,.csv,.pdf" onChange={(e) => handleUpload(e.target.files)} />
            </label>
          </div>
        }
      />
            ...
            {myFile ? (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-muted-foreground text-[11px]">{new Date(myFile.updated_at).toLocaleDateString()}</p>
                </div>
              </button>
            ) : (
                ...
                <div className="surface-card overflow-hidden">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b bg-secondary/40">
                          ...
                          {columns.map((col) => (
                            <th key={col} className="px-3 py-2.5 text-left text-xs font-semibold whitespace-nowrap">{col}</th>
                          ))}
                  ...
                  <div className="flex items-center justify-between border-t px-4 py-3">
                    {isOwner ? (
                      ...
                      <p className="text-muted-foreground text-xs">You can view and use this file but only the owner can edit it.</p>
                    )}
                  </div>
                </div>
              )}
