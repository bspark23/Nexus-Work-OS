...
function TasksPage() {
  ...
  // View task details dialog (for employees to see full task info)
  const [viewDetailsOpen, setViewDetailsOpen] = useState(false);
  const [viewDetailsTask, setViewDetailsTask] = useState<Task | null>(null);
  ...
  async function submitAssign() {
    ...
    if (!draft.assigned_to) {
      toast.error("Select an employee to assign this task to");
      return;
    }
    setTaskBusy(true);
    try {
  ...
  async function submitEmployeeUpdate() {
    ...
    setLocalOverrides((prev) => ({
      ...
      [id]: {
        ...
        ...(nowDone ? { completed_at: completedDate || new Date().toISOString() } : {}),
      },
    }));
    setSubmitOpen(false);
    setSubmitDraft(null);
    ...
    try {
      await saveTask({
        ...
        progress: finalProgress,
        notes: fullNotes || null,
        review_status: nowDone ? "pending_review" : null,
        ...(nowDone ? { completed_at: completedDate || new Date().toISOString() } : {}),
      });
      if (nowDone && assignedBy) {
        await broadcast({
          ...
          body: `"${taskTitle}" was completed. Please review and approve.`,
          actorId: user.id,
          type: "task",
        });
      }
      if (status === "blocked") {
        await broadcast({
          departmentId: deptId ?? null,
          title: "Task blocked",
          body: `"${taskTitle}" is blocked: ${reason}`,
          actorId: user.id,
          type: "warning",
        });
      }
      await logActivity({
        ...
        description: `${profile?.full_name ?? "Employee"} updated "${taskTitle}" to ${status}${reason ? ` — ${reason}` : ""}`,
      });
      qc.invalidateQueries({ queryKey: ["tasks"] });
      setLocalOverrides((prev) => {
        const n = { ...prev };
  ...
  async function submitTransfer() {
    ...
    try {
      ...
      toast.success(
        `Task transferred to ${departments.find((d) => d.id === transferToDept)?.name}`,
      );
  ...
  async function approveTask(t: Task) {
    ...
    await broadcast({
      ...
      title: "Task approved — Great job!",
      body: `"${t.title}" has been reviewed and approved.`,
      actorId: user.id,
      type: "success",
    });
    await logActivity({
      ...
      description: `${profile?.full_name} approved task "${t.title}"`,
    });
  ...
  const columns = TASK_STATUSES.map((s) => ({
    ...s,
    items: filteredTasks.filter((t) => t.status === s.value),
  }));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={
          isAdmin
            ? "Overview and management of all tasks across departments."
            : isDeptAdmin
              ? "Department task management & oversight."
              : "Your assigned tasks & deliverables."
        }
        actions={
          canManage ? (
            <Button
      ...
      {tasks.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          {columns.map((c) => (
            <section
              key={c.value}
              className="surface-card animate-rise flex flex-col p-3.5 rounded-xl border"
            >
              ...
              <ul className="space-y-2.5">
                {c.items.map((t) => {
                  ...
                  return (
                    <li
                      key={t.id}
                      className={`rounded-xl border p-3 text-sm transition-all ${overdue ? "border-warning/40 bg-warning/5" : pendingReview ? "border-info/40 bg-info/5" : "bg-secondary/50 hover:bg-secondary"}`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex min-w-0 flex-1 items-start gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            {t.description && (
                              <p className="text-muted-foreground line-clamp-2 mt-0.5 text-[11px]">
                                {t.description}
                              </p>
                            )}
                          </div>
                      ...
                      {assigneeName && (
                        <div className="text-muted-foreground mt-1.5 flex items-center gap-1 text-[11px]">
                          <User2 className="size-3" /> {assigneeName}{" "}
                          {assigner && <span className="ml-1"> {assigner}</span>}
                        </div>
                      )}
                      ...
                      {pendingReview && (
                        <div className="mt-1.5 flex items-center gap-1 text-[11px] text-info font-medium">
                          <Send className="size-3" /> Submitted for review
                        </div>
                      )}
                      {approved && (
                        <div className="mt-1.5 flex items-center gap-1 text-[11px] text-success font-medium">
                          <ThumbsUp className="size-3" /> Approved
                        </div>
                      )}
                      {needsRevision && (
                        <div className="mt-1.5 flex items-center gap-1 text-[11px] text-warning font-medium">
                          <TriangleAlert className="size-3" /> Needs revision
                        </div>
                      )}
                      ...
                      <div className="mt-2 flex items-center justify-between gap-2">
                        ...
                        />
                        <span
                          className={`text-[11px] ${overdue ? "text-warning font-medium" : "text-muted-foreground"}`}
                        >
                          ...
                          <>
                            <div className="grid grid-cols-2 gap-1.5">
                              ...
                              <Button
                                ...
                                className="h-7 text-[11px] px-2 gap-1"
                                onClick={() => {
                                  setReassignTask(t as Task);
                                  setReassignDept(t.department_id ?? "");
                                  setReassignTo("");
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-7 w-full text-[11px] px-2 gap-1 justify-center"
                              onClick={() => {
                                setTransferTask(t as Task);
                                setTransferToDept("");
                            ...
                            {pendingReview && (
                              <Button
                                size="sm"
                                className="h-7 w-full text-[11px] bg-info/20 text-info border-info/30 hover:bg-info/30"
                                variant="outline"
                                onClick={() => {
                                  setReviewTask(t as any);
                                  setReviewOpen(true);
                                }}
                              >
                          ...
                          isOwner && (
                            <div className="space-y-1.5">
                              <Button
                                size="sm"
                                variant="outline"
                                className="h-7 w-full text-[11px] px-2 gap-1"
                                onClick={() => {
                                  setViewDetailsTask(t as Task);
                                  setViewDetailsOpen(true);
                                }}
                              >
                                <Eye className="size-3 shrink-0" /> View Details
                              </Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{draft.id ? "Edit Task" : "Assign New Task"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            {isAdmin && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setDraft({ ...draft, department_id: v || null, assigned_to: null })
                  }
                >
                  <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <UserPicker
                people={deptPeople}
                value={draft.assigned_to ?? null}
                onChange={(id) => setDraft({ ...draft, assigned_to: id, owner_id: id ?? "" })}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDraft({ ...draft, expected_delivery_date: e.target.value || null })
                  }
                />
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={taskBusy || !draft.title || !draft.assigned_to}
            >
          ...
          {submitDraft && (
            <div className="space-y-4">
              <p className="text-muted-foreground text-sm font-medium truncate">
                "{submitDraft.taskTitle}"
              </p>
              <div className="space-y-2">
                <Label>Status *</Label>
                <Select
                  value={submitDraft.status}
                  onValueChange={(v) => setSubmitDraft({ ...submitDraft, status: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {TASK_STATUSES.filter((s) => s.value !== "expired").map((s) => (
                      <SelectItem key={s.value} value={s.value}>
                        {s.label}
                      </SelectItem>
                    ))}
                ...
                <div className="space-y-2">
                  <Label className="text-destructive">
                    ...
                    <span className="text-muted-foreground font-normal text-xs">
                      (required for {submitDraft.status})
                    </span>
                  </Label>
                  <Textarea
                    rows={3}
                    value={submitDraft.reason}
                    onChange={(e) => setSubmitDraft({ ...submitDraft, reason: e.target.value })}
                    placeholder={
                      submitDraft.status === "blocked"
              ...
              <div className="space-y-2">
                ...
                <Slider
                  value={[submitDraft.progress]}
                  max={100}
                  step={5}
                  onValueChange={([v]) => setSubmitDraft({ ...submitDraft, progress: v ?? 0 })}
                />
              </div>
              <div className="space-y-2">
                ...
                <>
                  <div className="rounded-xl border border-success/30 bg-success/5 p-3 space-y-3">
                    <p className="text-success text-xs font-medium">
                      {" "}
                      Marking as Done fill in completion details
                    </p>
                    <div className="space-y-2">
                      <Label>Completion note *</Label>
                      <Textarea
                        rows={3}
                        value={submitDraft.completionNote}
                        onChange={(e) =>
                          setSubmitDraft({ ...submitDraft, completionNote: e.target.value })
                        }
                        placeholder="Describe what you completed and how"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        type="date"
                        value={submitDraft.completedDate}
                        onChange={(e) =>
                          setSubmitDraft({ ...submitDraft, completedDate: e.target.value })
                        }
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        value={submitDraft.deliveryLink}
                        onChange={(e) =>
                          setSubmitDraft({ ...submitDraft, deliveryLink: e.target.value })
                        }
      ...
      <Dialog open={viewDetailsOpen} onOpenChange={setViewDetailsOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          {viewDetailsTask && (
            <div className="space-y-4">
              <div>
                <p className="text-xs text-muted-foreground font-medium mb-1">Task Title</p>
                <p className="font-semibold text-base leading-tight">{viewDetailsTask.title}</p>
              </div>

              {viewDetailsTask.description && (
                <div className="space-y-1.5">
                  <p className="text-xs text-muted-foreground font-medium">Description</p>
                  <div className="rounded-lg bg-secondary/50 p-3 text-sm whitespace-pre-wrap leading-relaxed">
                    {viewDetailsTask.description}
                  </div>
                </div>
              )}

              {viewDetailsTask.notes && (
                <div className="space-y-1.5">
                  <p className="text-xs text-muted-foreground font-medium">Notes / Instructions</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <StatusBadge
                    label={labelOf(TASK_STATUSES, viewDetailsTask.status)}
                    tone={toneOf(TASK_STATUSES, viewDetailsTask.status)}
                  />
                </div>
                <div>
                  <p className="text-xs text-muted-foreground font-medium mb-1">Priority</p>
                  <StatusBadge
                    label={labelOf(PRIORITIES, viewDetailsTask.priority)}
                    tone={toneOf(PRIORITIES, viewDetailsTask.priority)}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <Progress value={viewDetailsTask.progress} className="h-2 flex-1" />
                  <span className="text-xs font-semibold min-w-[36px] text-right">
                    {viewDetailsTask.progress}%
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t">
                <div>
                  ...
                  <p className="text-sm">{formatDate(viewDetailsTask.start_date)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground font-medium mb-1">Due Date</p>
                  <p
                    className={`text-sm ${isOverdue(viewDetailsTask) ? "text-warning font-medium" : ""}`}
                  >
                    {formatDate(viewDetailsTask.due_date)}
                  </p>
                </div>
                <div className="col-span-2">
                  ...
                  <p className="text-sm">{formatDate(viewDetailsTask.expected_delivery_date)}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t">
                <div>
                  ...
                  <p className="text-sm flex items-center gap-1">
                    ...
                    {allProfiles.find((p) => p.id === viewDetailsTask.owner_id)?.full_name ??
                      allProfiles.find((p) => p.id === viewDetailsTask.assigned_to)?.full_name ??
                      "—"}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground font-medium mb-1">Assigned By</p>
                  <p className="text-sm flex items-center gap-1">
                    <User2 className="size-3.5 text-muted-foreground" />
                    {allProfiles.find((p) => p.id === viewDetailsTask.assigned_by)?.full_name ??
                      "—"}
                  </p>
                </div>
                <div className="col-span-2">
                  ...
                  <p className="text-sm">
                    {departments.find((d) => d.id === viewDetailsTask.department_id)?.name ?? "—"}
                  </p>
                </div>
              </div>

              {(viewDetailsTask as any).review_status && (
                <div className="pt-2 border-t">
                  <p className="text-xs text-muted-foreground font-medium mb-1">Review Status</p>
                  {(viewDetailsTask as any).review_status === "pending_review" && (
                    <div className="flex items-center gap-1 text-info font-medium text-sm">
                      <Send className="size-3.5" /> Submitted for review
                    </div>
                  )}
                  {(viewDetailsTask as any).review_status === "approved" && (
                    <div className="flex items-center gap-1 text-success font-medium text-sm">
                      <ThumbsUp className="size-3.5" /> Approved
                    </div>
                  )}
                  {(viewDetailsTask as any).review_status === "needs_revision" && (
                    <div className="flex items-center gap-1 text-warning font-medium text-sm">
                      <TriangleAlert className="size-3.5" /> Needs revision
                    </div>
                  )}
                </div>
              )}

              {viewDetailsTask.completed_at && (
                <div className="pt-2 border-t">
                  <p className="text-xs text-muted-foreground font-medium mb-1">Completed At</p>
                  <p className="text-sm">{formatDate(viewDetailsTask.completed_at)}</p>
                </div>
              )}
            </div>
          )}
          <DialogFooter>
            ...
            {viewDetailsTask &&
              viewDetailsTask.owner_id === user?.id &&
              !(viewDetailsTask as any).review_status && (
                <Button
                  onClick={() => {
                    const t = viewDetailsTask;
                    setSubmitDraft({
                      id: t.id,
                      status: t.status,
                      progress: t.progress,
            ...
            <div className="space-y-4">
              ...
              {isAdmin && (
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Same or different dept…" />
                    </SelectTrigger>
                    <SelectContent>
                      {departments.map((d) => (
                        <SelectItem key={d.id} value={d.id}>
      ...
      <Dialog open={transferOpen} onOpenChange={setTransferOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              <ArrowRightLeft className="inline size-4 mr-2" />
          ...
          {transferTask && (
            <div className="space-y-4">
              ...
              <p className="text-muted-foreground text-xs">
                The department admin of the target department will receive this task.
              </p>
              <div className="space-y-2">
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          {task.notes && (
            <div className="rounded-xl bg-secondary/50 p-3 text-sm whitespace-pre-wrap">
              {task.notes}
            </div>
          )}
          <div className="space-y-2">
        ...
        <DialogFooter className="gap-2">
          ...
          <Button
            ...
            onClick={() => onReject(task, revisionNote)}
          >
